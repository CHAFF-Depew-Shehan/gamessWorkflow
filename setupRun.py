...
def processNodes(rxn, nodesTextFile, statusNodes):
    # Read nodes.txt for specified node and return all the necessary inputs for the createInputFile.py functions
    newInputFile = []
    for node in statusNodes:
        nodeInfo = []
        with open(nodesTextFile, "r") as atf:
            line = atf.readline()
        ...
        nodeVal = evalNodeSections(rxn,nodeInfo,N)
        if nodeVal:
...
def evalNodeSections(rxn, nodeInfo, N):
    inpFile = None
